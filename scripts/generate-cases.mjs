...
const HEART = {
  normal: { label: 'Normal Kalp Sesleri', timing: 'sistol+diyastol (S1–S2)', head: 'diaphragm', diag: null },
  s3: { label: 'S3 (Üçüncü Kalp Sesi)', timing: 'erken diyastol', head: 'bell', diag: null },
  s4: { label: 'S4 (Dördüncü Kalp Sesi)', timing: 'geç diyastol (presistol)', head: 'bell', diag: null },
  early_systolic_murmur: { label: 'Erken Sistolik Üfürüm', timing: 'erken sistol', head: 'diaphragm', diag: null },
...
const LUNG = {
  ...
  wheezing: { label: 'Wheezing', phase: 'ekspiryumda belirgin, sürekli müzikal', head: 'diaphragm' },
  rhonchi: { label: 'Ronküs', phase: 'her iki fazda, öksürükle değişebilir', head: 'diaphragm' },
  fine_crackles: { label: 'İnce Raller (Fine Crackles)', phase: 'inspirasyon sonunda', head: 'diaphragm' },
  coarse_crackles: { label: 'Kaba Raller (Coarse Crackles)', phase: 'inspirasyon boyunca', head: 'diaphragm' },
  pleural_rub: { label: 'Plevral Frotman', phase: 'iki fazlı (inspirasyon + ekspiryum)', head: 'diaphragm' },
}
...
const libByKey = {}
for (const g of library.groups) for (const it of g.items) libByKey[it.acousticFinding] = it
...
function caseWeights(hasDiag) {
  return hasDiag
    ? { technique: 20, localization: 20, recognition: 35, interpretation: 10, diagnosis: 10, systematic: 5 }
    : { technique: 20, localization: 25, recognition: 40, interpretation: 15, diagnosis: 0, systematic: 0 }
}
...
for (const [key, def] of Object.entries(HEART)) {
  ...
  for (const primary of locations) {
    ...
    q.push({
      ...
      correct: ['a'],
      feedbackCorrect: `Doğru. Kayıt "${def.label}" sınıfındandır. ${lib?.description ?? ''}`,
      feedbackIncorrect: `Bu kayıt "${def.label}" sınıfındandır. ${lib?.metaphor ?? ''}`,
      hint: lib?.metaphor,
    })
    if (locations.length > 1) {
      const others = locations.filter((l) => l !== primary).slice(0, 3)
      q.push({
        id: 'q2', type: 'localization', domain: 'localization',
        prompt: 'Bu vakanın ana kaydı hangi odakta alınmıştır?',
        options: [{ id: 'a', label: pointLabels[primary] }, ...others.map((l, i) => ({ id: String.fromCharCode(98 + i), label: pointLabels[l] }))],
        correct: ['a'],
        feedbackCorrect: `Doğru. Bu vaka kaydı ${pointLabels[primary]} bölgesindendir; diğer odaklarla karşılaştırın.`,
        feedbackIncorrect: `Bu vaka kaydı ${pointLabels[primary]} bölgesinden alınmıştır.`,
      })
    }
    q.push({
      ...
      correct: ['a'],
      feedbackCorrect: `Doğru. ${def.label} için tipik zamanlama: ${def.timing}.`,
      feedbackIncorrect: `Bu bulgunun zamanlaması: ${def.timing}.`,
    })
    q.push({
    ...
    if (def.diag) {
      q.push({
        ...
        correct: ['a'],
        feedbackCorrect: `Doğru. ${def.diag} ile uyumludur (veri seti sınıfı doğrulanmıştır).`,
        feedbackIncorrect: `Bu kayıt ${def.diag} sınıfıyla uyumludur; kesin tanı klinik bağlam ve EKG ile desteklenir.`,
      })
    }
    ...
    out.push({
      ...
      population: 'yetişkin',
      patient: { age, sex },
      chiefComplaint: 'Kalp seslerinin değerlendirilmesi istendi',
      history: 'Fizik muayenede kardiyak oskültasyon planlandı; tüm odaklar sistematik dinlenmelidir.',
  ...
  for (const primary of locations) {
    ...
    const q = [
      ...
      },
      {
        ...
        correct: ['a'],
        feedbackCorrect: `Doğru. ${def.label} için tipik özellik: ${def.phase}.`,
        feedbackIncorrect: `Bu bulgunun tipik özelliği: ${def.phase}.`,
      },
    ]
    if (locations.length > 1) {
      const others = locations.filter((l) => l !== primary).slice(0, 3)
      q.push({
        id: 'q3', type: 'localization', domain: 'localization',
        prompt: 'Bu vakanın ana kaydı hangi bölgeden alınmıştır?',
        options: [{ id: 'a', label: pointLabels[primary] }, ...others.map((l, i) => ({ id: String.fromCharCode(98 + i), label: pointLabels[l] }))],
        correct: ['a'],
        feedbackCorrect: `Doğru. Ana kayıt ${pointLabels[primary]} bölgesindendir.`,
        feedbackIncorrect: `Ana kayıt ${pointLabels[primary]} bölgesinden alınmıştır.`,
      })
    }
    out.push({
      id, title: `${def.label} — ${pointLabels[primary]}`,
      ...
      chiefComplaint: 'Solunum seslerinin değerlendirilmesi istendi',
      history: 'Bilateral karşılaştırmalı oskültasyon planlandı.',
      ...
      tasks: ['Bölgeleri bilateral dinleyin.', 'Bulguyu tanımlayın.', 'Faz özelliğini belirleyin.'],
      views: ['front', 'back'],
      allowedHeads: ['bell', 'diaphragm'],
      soundAssignments: assignments,
      primaryAcousticFinding: key,
...
// ---------------- KOMBİNE (MIXED) VAKALAR — veri setinin sınırları ----------------
{
  ...
  for (const [k, recs] of byKey) {
    ...
    out.push({
      ...
      chiefComplaint: 'Kalp ve akciğer sesleri birlikte değerlendirilecek',
      ...
      questions: [
        {
          ...
          correct: ['a'],
          feedbackCorrect: `Doğru. Kalp bileşeni: ${HEART[hf]?.label ?? hf}.`,
          feedbackIncorrect: `Kalp bileşeni ${HEART[hf]?.label ?? hf} ile uyumludur.`,
          hint: libByKey[hf]?.metaphor,
        },
        {
          ...
          correct: ['a'],
          feedbackCorrect: `Doğru. Akciğer bileşeni: ${LUNG[lf]?.label ?? lf}.`,
          feedbackIncorrect: `Akciğer bileşeni ${LUNG[lf]?.label ?? lf} ile uyumludur.`,
        },
      ],
  ...
  for (const [finding, recs] of byFinding) {
    ...
    for (const r of recs) {
      ...
      out.push({
        ...
        tasks: ['Odakları sistematik dinleyin.', 'Üfürümün zamanlamasını belirleyin.', 'Pediatrik bağlamı değerlendirin.'],
        views: ['front'],
        allowedHeads: ['bell', 'diaphragm'],
        soundAssignments: assignments,
        ...
        questions: [
          {
            ...
            feedbackIncorrect: `Bu kayıt "${HEART[finding]?.label ?? finding}" sınıfındandır. ${ctx.note}`,
            hint: libByKey[finding]?.metaphor,
          },
          {
            id: 'q2', type: 'single_choice', domain: 'interpretation',
            prompt: 'Bu yaş grubunda beklenen istirahat kalp hızı aralığı hangisidir?',
            options: [{ id: 'a', label: `${ctx.hr >= 140 ? '90–160' : ctx.hr >= 100 ? '80–140' : ctx.hr >= 80 ? '70–120' : '60–100'}/dk` }, { id: 'b', label: '40–60/dk' }, { id: 'c', label: '140–200/dk' }, { id: 'd', label: '30–50/dk' }],
            correct: ['a'],
            feedbackCorrect: `Doğru. ${ctx.note}`,
